defmodule SymphonyElixir.CoreTest do
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "`Workflow Review` -> PR is attached and validated"
    assert prompt =~ "This is a non-active state; Symphony must not dispatch a local agent for it"
    assert prompt =~ "Workflow Review re-entry protocol"
    assert prompt =~ "Fetch unresolved GitHub review threads through GraphQL"
    assert prompt =~ "Read current fallback findings from the existing `pr-reviewer-state` comment"
    assert prompt =~ "post a justified pushback reply beginning with `[codex]`"
    assert prompt =~ "move the issue back to `Workflow Review`"
    assert prompt =~ "Normal completion must not move directly from `In Progress` to `Human Review`"
    assert prompt =~ "Move directly to `Human Review` only for blocker, ambiguity, task defect"
    assert prompt =~ "Normal prReviewer medium/high feedback returns through `Workflow Review` -> `Todo`, not `Rework`"
