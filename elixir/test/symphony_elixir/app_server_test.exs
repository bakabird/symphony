defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server sanitizes invalid UTF-8 before encoding prompts and tool results" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-sanitize-json-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-sanitize-json.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-sanitize-json.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' "$line" >> "$trace_file"
        ...
        case "$count" in
          ...
          2)
            ;;
          3)
            printf '%s\\n' '{"id":2,"result":{"thread":{"id":"thread-sanitize"}}}'
            ;;
          4)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-sanitize"}}}'
            printf '%s\\n' '{"id":104,"method":"item/tool/call","params":{"tool":"custom_tool","callId":"call-sanitize","threadId":"thread-sanitize","turnId":"turn-sanitize","arguments":{}}}'
            ;;
          5)
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
      ...
      issue = %Issue{
        id: "issue-sanitize-json",
        identifier: "MT-94",
        title: "Sanitize JSON encoding",
        description: "Ensure outbound JSON survives invalid UTF-8",
        state: "In Progress",
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, prompt, issue, tool_executor: tool_executor)

      assert_received {:tool_called, "custom_tool", %{}}
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 payload["id"] == 104 and
                   Jason.decode!(payload["result"]["output"]) == %{
                     "data" => %{"value" => <<"bad-", 0xEF, 0xBF, 0xBD>>},
                     "success" => true
                   }
               else
                 false
